...
RegisterServerEvent('AnnonceOuvertAvocat')
AddEventHandler('AnnonceOuvertAvocat', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local xPlayers	= ESX.GetPlayers()
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Cabinet Avocat', '~b~Annonce', 'Le Cabinet est désormais ouvert!', 'CHAR_ORTEGA', 8)
	end
end)

RegisterServerEvent('AnnonceFermerAvocat')
AddEventHandler('AnnonceFermerAvocat', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local xPlayers	= ESX.GetPlayers()
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Cabinet Avocat', '~b~Annonce', 'Le Cabinet est désormais fermé à plus tard !', 'CHAR_ORTEGA', 8)
	end
end)
